...
var toHandlerKey2 = cacheStringFunction((str) => str ? `on${capitalize2(str)}` : ``);
    ...
    if (p.type === 6) {
      ...
      if (p.name === "bind" && isStaticArgOf(p.arg, "name")) {
        ...
        if (p.name === "bind" && p.arg && isStaticExp(p.arg)) {
          p.arg.content = camelize3(p.arg.content);
        }
        nonNameProps.push(p);
      }
    }
...
var transformOn = (dir, node, context, augmentor) => {
  ...
  if (arg.type === 4) {
    if (arg.isStatic) {
      ...
      eventName = createSimpleExpression(toHandlerKey2(camelize2(rawName)), true, arg.loc);
    } else {
...
var camelizeRE3 = /-(\w)/g;
var camelize4 = cacheStringFunction3((str) => {
...
var isRawTextContainer = makeMap2("style,iframe,script,noscript", true);
var parserOptions = {
...
var isEventOptionModifier = makeMap2(`passive,once,capture`);
var isNonKeyModifier = makeMap2(
  // event propagation management
  `stop,prevent,self,ctrl,shift,alt,meta,exact,middle`
);
var maybeKeyModifier = makeMap2("left,right");
var isKeyboardEvent = makeMap2(`onkeyup,onkeydown,onkeypress`, true);
...
var transformOn2 = (dir, node, context) => {
  return transformOn(dir, node, context, (baseResult) => {
    ...
    if (eventOptionModifiers.length) {
      const modifierPostfix = eventOptionModifiers.map(capitalize3).join("");
...
var camelizeRE4 = /-(\w)/g;
var camelize5 = cacheStringFunction4((str) => {
...
var toHandlerKey4 = cacheStringFunction4((str) => str ? `on${capitalize4(str)}` : ``);
...
function compileToFunction(template, options) {
  ...
  const { code } = compile(template, extend3({
    hoistStatic: true,
    onError: true ? onError : void 0,
    onWarn: true ? (e) => onError(e, true) : NOOP2
  }, options));
